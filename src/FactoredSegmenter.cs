...
namespace Microsoft.MT.Common.Tokenization
{
    ...
    internal struct Token
    {
        ...
        {
            if (!IsClassLemmaOrSomePunc(s)) // CLASS_LEMMA_ should not be touched
                if (CLASS_SEPARATOR != '@') // new form stores all-caps, for better readability
                    s = s.ToLowerInvariant();
            // @TODO: Get rid of this lower-casing. It should only affect the routine that applies the
            //        cap factor (SurfaceForm()), so we could just move it there.
            //        (And it also affects the \x \u parser right below)
    ...
    namespace Test
    {
        ...
        public class FactoredSegmenterTests
        {
            ...
            [TestMethod]
            [DeploymentItem(@"Segmenter\Test\", @"Segmenter\Test")]
